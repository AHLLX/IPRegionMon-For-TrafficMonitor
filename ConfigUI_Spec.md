...
### 📌 页面 4：当前状态与风险报告 (Current Status)
用于展示最新请求解析到的信息，包括直连与代理的公网 IP 详细情况，特别是由于网络污染导致的风险问题：
- **直连网络状态**：
  - 公网 IP 及所在地
  - ISP 提供商名称
  - 网络类型（家庭宽带、数据中心等）及安全性纯净度标记
- **代理网络状态**：
  - 出口 IP 及其真实所在地
  - 服务商
  - **泄漏检测与纯净度**：展示是否被识别为代理/VPN/Tor（即 ipleak 检测），以及对应的**欺诈风险分数 (Fraud Score)**。
